...
export default function ProjLogo({ logoName, logoLink, look }) {
  const { t } = useTranslation()
  const [isHovered, setIsHovered] = useState(false)

  return (
    <section className={`relative w-screen overflow-hidden px-4 ${look === 'dark' ? 'bg-text text-bg' : 'bg-bg text-text'}`}>
      <BgNoise />

      <div
        className={`relative flex justify-center items-center w-full h-100 border-t  ${look === 'dark' ? 'border-bgb' : 'border-main/50'}`}
      >
